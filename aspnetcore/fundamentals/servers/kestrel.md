...
[HTTP/2](https://httpwg.org/specs/rfc7540.html) is available for ASP.NET Core apps if the base requirements are met:
...
`MaxFrameSize` indicates the size of the largest frame payload that is allowed to be received for HTTP/2 connections. The value is provided in octets and must be between 2^14 and 2^24-1.
...
The following example permits HTTP/1.1 and HTTP/2 connections. Connections are secured by TLS with a supplied certificate:
...
private class TlsFilterAdapter : IConnectionAdapter
{
    ...
    public Task<IAdaptedConnection> OnConnectionAsync(ConnectionAdapterContext context)
    {
        ...
        // Throw NotSupportedException for any CipherAlgorithmType that you don't 
        // wish to support. Alternatively, define and compare ITlsHandshakeFeature to 
        // a list of acceptable cipher suites. CipherAlgorithmType.Null indicates that 
